...
use indracore_service::indracore_runtime as sel_rt;
use indracore_service::xelendra_runtime as xel_rt;
...
pub fn run_archive<D: ReadOnlyDB + 'static>(config: Config) -> Result<Box<dyn Archive<Block, D>>> {
	let mut db_path = if let Some(p) = config.indracore_path() {
		p
	} else {
	...
	let last_path_part =
		db_path.file_name().context("indracore path not valid")?.to_str().context("could not convert path to string")?;

	match last_path_part {
		"indracore" => db_path.push(format!("chains/{}/db", spec.id())),
	...
	match config.cli().chain.to_ascii_lowercase().as_str() {
		"xelendra" | "xel" => {
			let archive = ArchiveBuilder::<Block, xel_rt::RuntimeApi, indracore_service::XelendraExecutor, D>::default()
				.chain_spec(spec)
		...
		"indracore-local" => {
			let archive = ArchiveBuilder::<Block, sel_rt::RuntimeApi, indracore_service::IndracoreExecutor, D>::default()
				.chain_spec(spec)
				.chain_data_path(Some(db_path))
				.pg_url(config.psql_conf().map(|u| u.url()))
				.cache_size(config.cache_size())
